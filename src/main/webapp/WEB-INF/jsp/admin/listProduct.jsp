...
<c:forEach items="${p.productimage}" var="pi" >
							...
							<img width="40px" src="img/productSingle/${pi.pid}.jpg">
						
</c:forEach>
					</td>
			 		<%-- <td>${fn:substring(p.name, 0, 50)}</td> --%>
			 		<td>${p.name}</td>
					<td>${p.subtitle}</td>
					...
					<td><a deleteLink="true"
						   href="admin_product_delete?id=${p.id}?pn=${pageInfo.pageNum}&cid=${c.id}"><span
...
<%@include file="../include/admin/listProductPage.jsp"%>
	...
	<div class="panel panel-warning addDiv">
		...
		<div class="panel-body">
			<form method="post" id="addForm" action="admin_product_add?pn=${pageInfo.pageNum}&cid=${c.id}">
